...
competencies <- list(
  Strategic = c("Section3_StrategicThinkingi.", "Section3_StrategicThinkingii.", "Section3_StrategicThinkingiii."),
  Systems = c("Section3_SystemsThinkingi.", "Section3_SystemsThinkingii.", "Section3_SystemsThinkingiii."),
  Future = c("Section3_FutureThinkingi.", "Section3_FutureThinkingii.", "Section3_FutureThinkingiii."),
  Values = c("Section3_ValuesThinkingi.", "Section3_ValuesThinkingii.", "Section3_ValuesThinkingiii.")
)
...
compute_reliability <- function(data) {
  map_dfr(names(competencies), function(comp) {
    tryCatch({
      data.frame(
        Competency = comp,
        pre_alpha = round(alpha(data$pre[competencies[[comp]]])$total$raw_alpha, 2),
        post_alpha = round(alpha(data$post[competencies[[comp]]])$total$raw_alpha, 2)
      )
    }, error = function(e) {
      warning(paste("Error in computing reliability for", comp, ":", e$message))
      return(data.frame(Competency = comp, pre_alpha = NA, post_alpha = NA))
    })
  })
}
...
compute_general_stats <- function(data) {
  map_dfr(names(competencies), function(comp) {
    pre_scores <- rowSums(data$pre[competencies[[comp]]], na.rm = TRUE)
    post_scores <- rowSums(data$post[competencies[[comp]]], na.rm = TRUE)
...
perform_t_tests <- function(data, normality) {
  map_dfr(names(competencies), function(comp) {
    # Compute pre and post scores
    pre_scores <- rowSums(data$pre[competencies[[comp]]], na.rm = TRUE)
    post_scores <- rowSums(data$post[competencies[[comp]]], na.rm = TRUE)
    
    # Get normality p-values
    normality_row <- normality %>% filter(competency == comp)
...
perform_anova <- function(data, normality, pre_or_post) {
  map_dfr(names(competencies), function(comp) {
    scores <- rowSums(data[[pre_or_post]][competencies[[comp]]], na.rm = TRUE)
    ...
    pvalues <- map_dbl(groups, ~ run_test(scores, .x, data[[pre_or_post]], normality[[paste0(pre_or_post, "_pvalue")]][normality$competency == comp]))
    
    data.frame(
      Question = comp,
      Test_Type = ifelse(normality[[paste0(pre_or_post, "_pvalue")]][normality$competency == comp] >= 0.05, "ANOVA", "Kruskal-Wallis"),
...
perform_anova_majors <- function(predata, postdata, normality, items, pre_col = "pre_pvalue", post_col = "post_pvalue") {
  expand_and_test <- function(data, label, norm_col) {
    ...
    map_dfr(items, ~ {
      pval <- filter(normality, competency == .x)[[norm_col]]  # Extract p-value based on competency
      response_cols <- competencies[[.x]]  # Get the competency items (e.g., "Strategic")
      
      # Ensure summing the correct columns associated with the competency
      if (!all(response_cols %in% colnames(data_long))) {
        stop(paste("Some columns for competency", .x, "are missing in the data"))
      }
      
      response <- rowSums(data_long[response_cols], na.rm = TRUE)  # Sum the relevant columns
      test_type <- if (pval >= 0.05) "One-Way ANOVA" else "Kruskal-Wallis"
      
      p_value <- if (pval >= 0.05) {
        round(summary(aov(response ~ Major_All, data = data_long))[[1]]$`Pr(>F)`[1], 4)
      } else {
        round(kruskal.test(response ~ data_long$Major_All)$p.value, 4)
      }
      
      tibble(Time = label, Question = .x, Test_Type = test_type, P_Value.Major = p_value)
    })
  }
...
reliability_results <- compute_reliability(filtered_data)
...
anova_majors_results <- perform_anova_majors(filtered_predata, filtered_postdata, normality, names(competencies))

#---------------------------------------------------SAVE ALL RESULTS------------------------------------------------------------------------
wb <- createWorkbook()
list(gen_stats, reliability_results, normality, t_test_results, anova_pre_results, anova_post_results, anova_majors_results) %>%
  setNames(c('gen_stats', 'reliability_results', 'normality', 't_test_results', 'anova_pre_results', 'anova_post_results', 'anova_majors_results')) %>%
  walk2(names(.), ~ { addWorksheet(wb, .y); writeData(wb, .y, .x) })

saveWorkbook(wb, "./Results/overall_self_ratings_stats.xlsx", overwrite = TRUE)
